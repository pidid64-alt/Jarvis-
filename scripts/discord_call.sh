#!/bin/bash
# Universal Discord call script
# Usage: discord_call.sh <username>
...
xdotool type --window "$discord_window" --delay 50 "$username"
sleep 0.7
...
for i in {1..10}; do
    dm_window=$(xdotool search --class discord 2>/dev/null | while read win; do
        ...
        # Look for DM window with @ and reasonable size
        if [[ "$name" == *"@"* ]] && [ "$width" -gt 500 ]; then
            echo "$win"
            break
        fi
    done)
...
notify-send "Discord" "Звоню пользователю: $username"
echo "Calling $username in Discord"
exit 0
